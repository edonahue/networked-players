...
clique detection, tie-breaking) is thoroughly unit-tested with hand-verified
expectations in packages/graph-core/tests/test_marginal_evaluation.py; this
pins the CLI wiring only: baseline-release-id union from two input files,
output shape, and the local-only guard."""
...
def test_selects_by_score_when_marginal_value_ties_and_unions_both_baselines(
    ...
    baseline_catalog = tmp_path / "albums.v1.json"
    baseline_catalog.write_text(json.dumps({"albums": [{"main_release_id": 101}]}))
    additional_baseline = tmp_path / "editorial-seed.json"
    additional_baseline.write_text(json.dumps({"albums": [{"main_release_id": 105}]}))

    finalists_path = tmp_path / "finalists.json"
    finalists_path.write_text(
        json.dumps(
            [
                {"master_id": 1, "main_release_id": 103, "score": 5},
                {"master_id": 2, "main_release_id": 104, "score": 50},
            ]
        )
    )
...
def test_additional_baseline_is_optional(tmp_path: Path) -> None:
    ...
    baseline_catalog.write_text(json.dumps({"albums": [{"main_release_id": 101}]}))
    finalists_path = tmp_path / "finalists.json"
    finalists_path.write_text(json.dumps([{"master_id": 1, "main_release_id": 103}]))
...
def test_refuses_to_write_outside_local(tmp_path: Path) -> None:
    dataset = _write_full_dataset(tmp_path)
    baseline_catalog = tmp_path / "albums.v1.json"
    baseline_catalog.write_text(json.dumps({"albums": []}))
    finalists_path = tmp_path / "finalists.json"
    finalists_path.write_text(json.dumps([]))

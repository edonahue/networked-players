...
`connection_daily_manifest.py`, `record_routes.py`). Before this module,
nothing ever called all five together against the actual committed files
under `apps/web/public/data/**` -- CI validated the synthetic test fixture
(`apps/web/scripts/build-rounds.mjs --check`) and every dependency-free
validator's own unit tests, but never the real, currently-published JSON
itself. A defect in a real committed artifact (missing field, stale version,
dangling reference) could reach `main` and stay there undetected.

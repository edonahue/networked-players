...
def artifact_paths(tmp_path: Path) -> dict[str, Path]:
    paths = {
        ...
        "output": tmp_path / "contributors" / "album-hop-distances.v1.json",
    }
...
def test_build_album_hop_distances_cli_wiring(
    ...
    # A single 1-hop path: each of Alice/Bob is 0 hops from their own
    # endpoint album and 1 hop from the other -- 4 entries total.
    ...
    assert len(written["entries"]) == 4
    assert {"artist_id": 100, "album_id": "master-1", "hop_distance": 0} in written["entries"]
    assert {"artist_id": 100, "album_id": "master-2", "hop_distance": 1} in written["entries"]
    assert {"artist_id": 200, "album_id": "master-1", "hop_distance": 1} in written["entries"]
    assert {"artist_id": 200, "album_id": "master-2", "hop_distance": 0} in written["entries"]


def test_validate_album_hop_distances_cli_wiring(
    artifact_paths: dict[str, Path], capsys: pytest.CaptureFixture[str]
) -> None:
    main(
        [
    ...
    assert result == {"ok": True, "entries": 4}


def test_build_refuses_to_write_when_validation_fails(
    artifact_paths: dict[str, Path],
) -> None:
    # A contributor index that doesn't actually contain artist 100 or 200 --
    # every entry's artist_id fails the "published contributor" check, so
    # the build must refuse to write rather than emit an invalid artifact.
    artifact_paths["contributor_index"].write_text(json.dumps({"contributors": []}))

...
- **Connect Two Records** (`/play/connect/`): pick any two catalog albums and search for a real, documented record-to-record route between them — a genuine single-source/single-sink search over every credited contributor on each album, not just its primary artist (ADR 0058), with real endpoint and evidence cards and optional producer/engineer-, drums/bass-, or guitar-only role filters (ADR 0053). An unfiltered search is ranked, not just the first path found — evidence quality, hub dependence, and role substance decide among equal-hop candidates, with the plain shortest route and a genuinely distinct alternate still shown alongside it and a "Why this route?" disclosure explaining the pick (ADR 0059). The result is a shareable link (`?a=`/`?b=`/`?mode=`), a Swap control that reverses an already-found route without a new search, and cover art on each endpoint.

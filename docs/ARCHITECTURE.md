...
| Bounded ARM workers | Validation, cache audits, evidence shards, metadata preparation — including a dependency-free `artifact.validate` check per real public artifact (catalog, album-art registry, both game modes' pools, the daily manifest, contributor index, pathfinding graph), dispatched through the ADR 0034 capability platform (`select_worker()`/RQ), not a per-artifact deploy playbook | Three active Raspberry Pi 3B nodes, real live capability-platform participants |

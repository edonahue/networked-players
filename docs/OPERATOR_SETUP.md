...
| Pathfinding graph (ADR 0058/0059/0068/0071) | `apps/web/public/data/pathfinding/graph.v3.json` | `pathfinding_graph_version` (+ `catalog_version`, `graph_policy_version`) | One-hop dataset + catalog + album-credit-membership, performer-gated (`graph.py.GRAPH_POLICY_VERSION`) | `networked_players_contracts.pathfinding_graph::pathfinding_graph_failures` (schema-version-aware, accepts v1/v2/v3/v4) | `pathfinding-graph` | Connect Two Records, Network Explorer, the private research workbench, `scripts/submit_artifact_check.py`'s fleet default |
| Pathfinding graph v4, dual-live (ADR 0071) | `apps/web/public/data/pathfinding/graph.v4.json` | `pathfinding_graph_version` (+ `catalog_version`, `graph_policy_version`) | Same inputs as v3, `build-pathfinding-graph --schema-version 4` -- role-dictionary-encodes `edge_role_a`/`edge_role_b` (real measured effect: raw −41.5%, gzip −24.0%, see `data/contracts/pathfinding-graph-v4.md`) | Same validator, registered as the separate `pathfinding_graph_v4` group so `make check` catches a v4 defect independently of v3 | none yet (no real consumer fetches it; ad hoc via the `pathfinding-graph` validator's `--artifact` override if ever needed before cutover) | None yet -- published dual-live for `make check` validation ahead of the consumer cutover (Connect, Explore, the research workbench, the fleet default, in that order, then `graph.v3.json` retires as its own step) |

...
| Pathfinding graph (ADR 0058/0059/0068/0071) | `apps/web/public/data/pathfinding/graph.v3.json` | `pathfinding_graph_version` (+ `catalog_version`, `graph_policy_version`) | One-hop dataset + catalog + album-credit-membership, performer-gated (`graph.py.GRAPH_POLICY_VERSION`) | `networked_players_contracts.pathfinding_graph::pathfinding_graph_failures` (schema-version-aware, accepts v1/v2/v3/v4 -- `build-pathfinding-graph --schema-version 4` role-dictionary-encodes `edge_role_a`/`edge_role_b`, ADR 0071; not yet published, `graph.v3.json` is still the only real artifact) | `pathfinding-graph` | Connect Two Records, Network Explorer, the private research workbench, `scripts/submit_artifact_check.py`'s fleet default |

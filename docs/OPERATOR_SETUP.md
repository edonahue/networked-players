...
| Pathfinding graph v4 (ADR 0058/0059/0068/0071) | `apps/web/public/data/pathfinding/graph.v4.json` | `pathfinding_graph_version` (+ `catalog_version`, `graph_policy_version`) | `build-pathfinding-graph --schema-version 4` -- role-dictionary-encodes `edge_role_a`/`edge_role_b` (real measured effect: raw −41.5%, gzip −24.0% vs. v3 with byte-identical edges, see `data/contracts/pathfinding-graph-v4.md`) | `networked_players_contracts.pathfinding_graph::pathfinding_graph_failures` (schema-version-aware, accepts v1/v2/v3/v4) | `pathfinding-graph` (the fleet default as of the consumer cutover) | Connect Two Records, Network Explorer, the private research workbench, `scripts/submit_artifact_check.py`'s fleet default -- **every real consumer has now cut over** |
| Pathfinding graph v3, legacy (ADR 0058/0059/0068) | `apps/web/public/data/pathfinding/graph.v3.json` | `pathfinding_graph_version` (+ `catalog_version`, `graph_policy_version`) | Same inputs as v4, pre-dictionary-encoding shape | Same validator, registered as the separate `pathfinding_graph_v4`'s sibling `pathfinding_graph` group so `make check` catches a v3 defect independently of v4 | ad hoc via `--artifact apps/web/public/data/pathfinding/graph.v3.json --artifact apps/web/public/data/catalog/albums.v1.json` (no longer the default) | **None -- zero live traffic.** Kept published and validated only until an explicit retirement PR (ADR 0058's own precedent for v1's retirement) deletes it and collapses `pathfinding_graph_v4` back into the single `pathfinding_graph` group name. |

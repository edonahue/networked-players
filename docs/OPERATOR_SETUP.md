...
| Contributor index | `apps/web/public/data/contributors/index.v1.json` | `contributor_index_version` (+ `catalog_version`) | Already-published `challenge.v2.json` + Record Routes + evidence-release-registry artifacts | `networked_players_contracts.contributor_index::contributor_index_failures` | `contributor-index` | `/contributors/[id]/` pages, album page contributor links |
| Album-hop-distances (ADR 0048 addendum) | `apps/web/public/data/contributors/album-hop-distances.v1.json` | `album_hop_distances_version` (+ `catalog_version`) | Already-published `challenge.v2.json` + Record Routes, cross-validated against the companion contributor-index artifact's own `artist_id`s | `networked_players_contracts.album_hop_distances::album_hop_distances_failures` | none -- validated via `validate-public-artifacts` (CI/`make check`) only, no per-artifact Pi check | `/contributors/[id]/` pages (hop-distance labels on non-direct album connections) |
| Background-only-profiles (ADR 0048/0060 addendum) | `apps/web/public/data/contributors/background-only-profiles.v1.json` | `background_only_profiles_version` (+ `catalog_version`) | Already-published `challenge.v2.json` + Record Routes (full, uncapped role-text vocabulary per contributor), cross-validated against the companion contributor-index artifact's own `artist_id`s | `networked_players_contracts.background_only_profiles::background_only_profiles_failures` | none -- validated via `validate-public-artifacts` (CI/`make check`) only, no per-artifact Pi check | `/contributors/[id]/` and `/albums/[album]/` pages (muted styling on background-engineering-only connections) |
...
| Pathfinding graph (v3, ADR 0068) | `apps/web/public/data/pathfinding/graph.v3.json` | `pathfinding_graph_version` (+ `catalog_version`, `graph_policy_version`) | Same inputs as v2, performer-gated (`graph.py.GRAPH_POLICY_VERSION`) -- identical CSR/`album_virtual_nodes` shape, fewer real edges | `networked_players_contracts.pathfinding_graph::pathfinding_graph_failures` (schema-version-aware, accepts v1/v2/v3) | none yet -- validated via `validate-public-artifacts` (CI/`make check`) only; dual-live alongside v2, not yet fetched by any real consumer until the cutover PR flips Connect/Explore's default URL and retires v2 | none yet (dual-live, pre-cutover) |
| Album-centered challenge (v3, ADR 0068) | `apps/web/public/data/challenge.v3.json` | `provenance.catalog_version`, `provenance.graph_policy_version` (new) | Same inputs as `challenge.v2.json`, performer-gated -- `CHALLENGE_SCHEMA_VERSION` stays 2 (shape unchanged); `graph_policy_version`, not the schema version, is what distinguishes it | `networked_players_contracts.challenge::challenge_failures` | none -- validated via `validate-public-artifacts` (CI/`make check`) only, same as `challenge.v2.json`; dual-live, not yet fetched by any real consumer until the cutover PR | none yet (dual-live, pre-cutover) |

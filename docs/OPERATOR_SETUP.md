...
- Generate a real `challenge.v3.json` (`build-challenge-from-dump`).
- Retire `/demo/`.
- Deploy.
...
| Contributor index | `apps/web/public/data/contributors/index.v1.json` | `contributor_index_version` (+ `catalog_version`) | Already-published `challenge.v3.json` + Record Routes + evidence-release-registry artifacts | `networked_players_contracts.contributor_index::contributor_index_failures` | `contributor-index` | `/contributors/[id]/` pages, album page contributor links |
| Album-hop-distances (ADR 0048 addendum) | `apps/web/public/data/contributors/album-hop-distances.v1.json` | `album_hop_distances_version` (+ `catalog_version`) | Already-published `challenge.v3.json` + Record Routes, cross-validated against the companion contributor-index artifact's own `artist_id`s | `networked_players_contracts.album_hop_distances::album_hop_distances_failures` | none -- validated via `validate-public-artifacts` (CI/`make check`) only, no per-artifact Pi check | `/contributors/[id]/` pages (hop-distance labels on non-direct album connections) |
...
| Pathfinding graph (ADR 0058/0059/0068/0071) | `apps/web/public/data/pathfinding/graph.v4.json` | `pathfinding_graph_version` (+ `catalog_version`, `graph_policy_version`) | `build-pathfinding-graph` (schema-version 4 by default) -- role-dictionary-encodes `edge_role_a`/`edge_role_b` (real measured effect: raw −41.5%, gzip −24.0% vs. the retired v3 shape, with byte-identical edges; see `data/contracts/pathfinding-graph-v4.md`) | `networked_players_contracts.pathfinding_graph::pathfinding_graph_failures` (schema-version-aware, accepts v1/v2/v3/v4) | `pathfinding-graph` | Connect Two Records, Network Explorer, the private research workbench, `scripts/submit_artifact_check.py`'s fleet default |
| Prominence sidecar (graph-expansion Phase 1, plan section 8) | `apps/web/public/data/pathfinding/prominence.v1.json` | `prominence_version` (+ `catalog_version`, `pathfinding_graph_version`) | `build-prominence` -- node-aligned precomputed ranking signals (degree, albums_1hop/2hop, evidence_releases, role_diversity, first/last_year, rank) from the already-published pathfinding graph + evidence-release registry, never a fresh one-hop corpus query; see `data/contracts/prominence-v1.md` | `networked_players_contracts.prominence::prominence_failures` (validated against the pathfinding graph, not the catalog) | `prominence` | Network Explorer (`explorerStage.ts` neighbor ranking + paging) |
| Search index (graph-expansion Phase 1, plan section 7) | `apps/web/public/data/search/index.v1.json` | `search_index_version` (+ `catalog_version`, `contributor_index_version`) | `build-search-index` -- a flat list of searchable albums + contributors from the already-published catalog + contributor index, never a fresh corpus query; every entry's `state` is `present` (`candidate` entries need `catalog/candidates.v1.json`, a Phase 3 dependency); see `data/contracts/search-index-v1.md` | `networked_players_contracts.search_index::search_index_failures` (validated against both the catalog and the contributor index) | `search-index` | Connect Two Records' album picker (`siteSearch.ts`) -- a header search on Explore/Browse pages is separate, later work |
| Album-centered challenge (ADR 0012/0068) | `apps/web/public/data/challenge.v3.json` | `provenance.catalog_version`, `provenance.graph_policy_version` (new) | One-hop dataset + catalog, performer-gated -- `CHALLENGE_SCHEMA_VERSION` stays 2 (shape unchanged); `graph_policy_version`, not the schema version, is what distinguishes it | `networked_players_contracts.challenge::challenge_failures` | none -- validated via `validate-public-artifacts` (CI/`make check`) only, a one-shot static artifact with no independent `artifact_version` to re-verify | Homepage, `/albums/` grid + detail pages, `/contributors/` |

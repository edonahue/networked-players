...
The album grid, evidence viewer, and every game mode run on a real, Discogs monthly-dump-derived catalog (CC0-licensed): ${studioAlbumCount} studio albums, ${artistCount} artists, ${documentedConnectionCount} documented connections. Connection Guesser (${oneHopRoundCount} one-hop and ${twoHopRoundCount} two-hop rounds, plus a frozen Connection of the Day schedule), Connect Two Records (search any two catalog albums for a documented route through their credited contributors, with role-restricted modes), Record Routes (guess how many documented-credit hops connect two records), and Explore (an open-ended, bounded network view of one album's credit neighborhood) all draw from this same catalog and its evidence. A separate legacy demo at /demo/ runs on a small, curated, API-sourced Discogs subset predating the dump pipeline -- it is not the main catalog.

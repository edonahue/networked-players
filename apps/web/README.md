...
Both artifacts are real Discogs-shaped data. `challenge.v3.json` (the album grid
and play pages) is now real: 179 studio albums / 348 artists / 300 documented paths /
396 evidence releases (Phase 7's catalog expansion, PR #161), `snapshot=20260601`,
generated by `packages/graph-core`'s
...
is live for all 179 albums via a separately versioned art registry
...
`cover_image` inside `challenge.v3.json` itself is deliberately `null` for every album —
...
Real cover art (179/179 albums, following Phase 7's catalog expansion), Record Routes
(`/play/routes/`, a peer path-guessing mode from the same canonical catalog, ADR 0046),
and per-artifact Pi-fleet validation

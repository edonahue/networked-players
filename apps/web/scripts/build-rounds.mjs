#!/usr/bin/env node
// Expand the authored Meridian Tapes universe definition and derive a
// synthetic-only test fixture (docs/WEB_PRODUCT_PLAN.md §8, ADR 0037,
// ADR [connection-rounds ADR]). Two committed **test fixture** artifacts --
// NOT public production data:
//
//   tests/fixtures/game-universe.synthetic.v1.json
//   tests/fixtures/game-rounds.synthetic.v1.json
//
// The real Connection Guesser pool at public/data/game/{universe,rounds}.v1.json
// is generated by the Python catalog/graph-core pipeline
// (`networked-players-catalog build-connection-rounds`), not this script --
// see the ADR above for why production data moved off the JS generator.
// This script's only remaining job is the deterministic synthetic fixture a
// few tests still use (game-data.spec.ts's synthetic-pool checks); it must
// never write into public/.
//
// Modes:
//   --write   regenerate the fixture on disk
//   --check   (default) regenerate in memory, deep-compare against the
//             committed fixture, and run every validation; non-zero exit on
...
const UNIVERSE_PATH = join(
  WEB_ROOT,
  "tests/fixtures/game-universe.synthetic.v1.json",
);
const ROUNDS_PATH = join(
  WEB_ROOT,
  "tests/fixtures/game-rounds.synthetic.v1.json",
);
...
const CAPS = { synOneHop: 34, synTwoHop: 14 };
const MIN = { synthetic: 40, synTwoHop: 8 };
...
function validate(universe, rounds) {
  ...
  // Round correctness. This fixture is synthetic-only -- every round must
  // say so; a "real-records" round here would mean production data leaked
  // into the test fixture generator.
  ...
  for (const round of rounds.rounds) {
    if (round.pool !== "synthetic-universe")
      fail(
        problems,
        `round ${round.id} pool must be 'synthetic-universe' in this fixture, got '${round.pool}'`,
      );
  }
  const ids = new Set();
  for (const round of rounds.rounds) {
...
function buildAll() {
  ...
  const rounds = {
    ...
    provenance: {
      source:
        "Synthetic fixture -- the Meridian Tapes universe, an entirely fictional catalog invented for this repository, for tests only. The real Connection Guesser pool is generated separately (networked-players-catalog build-connection-rounds) and published at public/data/game/*.json.",
      license:
        "Fictional synthetic data authored in this repository; no external source, no real artists, releases, or Discogs identifiers",
      note: "Rounds are regenerated deterministically by apps/web/scripts/build-rounds.mjs; every distractor is validated to NOT satisfy the connection. Never presented as real catalog data, and never published outside this test fixture.",
      generated_by:
...
function main() {
  ...
  if (mode === "write") {
    ...
    console.log(
      `build-rounds: wrote synthetic test fixture -- ${universe.albums.length} albums, ` +
        `${universe.credits.length} credits, ${rounds.rounds.length} rounds ` +
        `(${rounds.rounds.filter((r) => r.kind === "two_hop").length} two-hop)`,
    );
    return;
  }

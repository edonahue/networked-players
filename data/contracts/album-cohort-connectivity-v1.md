...
| `scorer_version` | int | Version of the scoring/quality-flag logic (`SCORER_VERSION`), bumped on any material change. Currently 5 ([ADR 0068](../../docs/decisions/0068-performer-only-public-graph.md), performer-gated `credit_edges` traversal). Artifacts from version ≤ 3 came from a release-container graph in which any two artists on one compilation were one hop apart; version 4 added track-scoped `credit_edges` traversal ([ADR 0035](../../docs/decisions/0035-track-scoped-credit-edges.md)); version 5 gates `same_recording`/`release_scope` edges (and `performer_credit`/`non_performer_only` quality flags) on `is_performer_role`, a materially smaller edge relation. Hop counts and quality flags are **not comparable** across these version boundaries. |

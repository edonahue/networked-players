...
> **Retired (2026-08-09, ADR 0058).** `build-pathfinding-graph` only
> produces the v2 shape (see `pathfinding-graph-v2.md`) since Phase 4
> Slice 5. The v1 artifact (`apps/web/public/data/pathfinding/graph.v1.json`)
> stayed committed and live until both real browser consumers — Connect
> Two Records (Slice 7) and Network Explorer (retired alongside this doc
> update) — cut over to v2; it has now been deleted from the repository.
> `pathfinding_graph_failures` remains schema-version-aware and still
> accepts a v1-shaped payload for any historical export that needs
> re-validating, but no live artifact publishes it anymore. This doc is
> kept as a historical record, not regenerated or re-published.

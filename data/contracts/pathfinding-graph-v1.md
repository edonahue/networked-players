...
> **Superseded generator, still-live artifact.** As of ADR 0058 (Phase 4
> Slice 5), `build-pathfinding-graph` only produces the v2 shape (see
> `pathfinding-graph-v2.md`) — this v1 file stays committed and validated
> (`pathfinding_graph_failures` accepts both schema versions) until Connect
> Two Records actually cuts over to v2, at which point v1 retires. This doc
> describes the real, still-published v1 artifact as it exists today; it is
> not regenerated by the current builder.

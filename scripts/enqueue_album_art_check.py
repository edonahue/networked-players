#!/usr/bin/env python3
"""Enqueue a single album-art-registry validation check across the joined
Pi workers, via the jobs broker + RQ. Re-checks the already-published
album-art.v1.json/albums.v1.json pair deployed alongside the job body by
deploy-album-art-check-job.yml -- no dataset, no CreditGraph, safe to run at
any time.

Mirrors scripts/enqueue_connection_rounds_check.py's broker-connect,
per-worker-queue, burst-worker-launch, and wait-and-collect structure; see
...
pair check has no natural sharding dimension.
...
(deploy-album-art-check-job.yml).

Not meant to be invoked directly -- use scripts/enqueue-album-art-check.sh
(or `make album-art-check-distributed`), which sources
...
QUEUE_PREFIX = "album-art-check"
JOB_TIMEOUT_S = 60
WAIT_TIMEOUT_S = 120.0
JOB_FUNCTION = "album_art_check_job.check_album_art"
# Filenames only: deploy-album-art-check-job.yml copies both artifacts into
# the same persistent rq_jobs_dir as the job body itself, and
# album_art_check_job.py resolves relative paths against that directory
# (see its _resolve()).
REGISTRY_FILENAME = "album-art.v1.json"
...
def require_env(name: str) -> str:
    ...
    if not value:
        print(
            f"ABORT: set {name} (see scripts/enqueue-album-art-check.sh).",
            file=sys.stderr,
        )
        raise SystemExit(1)
    return value
...
def main() -> None:
    ...
    print(f"==> Checking {REGISTRY_FILENAME}/{CATALOG_FILENAME} via {worker}.")
    job = queue.enqueue(
        JOB_FUNCTION,
        REGISTRY_FILENAME,
        CATALOG_FILENAME,
    ...
    record: dict[str, Any] = {
        ...
        "registry_artifact": REGISTRY_FILENAME,
        "catalog_artifact": CATALOG_FILENAME,
        "worker": worker,
    ...
    output_path = OUTPUT_DIR / f"album-art-check-{timestamp}.json"
    output_path.write_text(json.dumps(record, indent=2, default=str) + "\n")

    print(f"==> Wrote {output_path}.")
    if record["ok"]:
        print("==> PASS: album-art registry is valid.")
    else:

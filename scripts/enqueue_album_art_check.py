#!/usr/bin/env python3
"""Enqueue an album-art-registry validation check across every targeted Pi
worker (fanned out one job per worker, aggregated pass/fail), via the jobs
broker + RQ. Re-checks the already-published
...
USAGE_HINT = "scripts/enqueue-album-art-check.sh"
...
def main() -> None:
    ...
    print(
        f"==> Checking {REGISTRY_FILENAME}/{CATALOG_FILENAME} across "
    ...
    per_worker = enqueue_and_collect(
        ...
        job_args=(REGISTRY_FILENAME, CATALOG_FILENAME),
        job_timeout=JOB_TIMEOUT_S,
        queue_factory=lambda name: Queue(name, connection=redis_conn),
    )
    output_path = write_report(
        prefix=QUEUE_PREFIX,
        extra={"registry_artifact": REGISTRY_FILENAME, "catalog_artifact": CATALOG_FILENAME},
        per_worker=per_worker,
    )

    print(f"==> Wrote {output_path}.")
    if all(v["ok"] for v in per_worker.values()):
        print(f"==> PASS: album-art registry is valid on all {len(per_worker)} worker(s).")
    else:

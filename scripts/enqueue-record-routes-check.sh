#!/usr/bin/env bash
#
# Enqueue a single Record Routes validation check across the joined Pi
...
# actual driver (scripts/enqueue_record_routes_check.py).
#
...
#   - infra/ansible/run-deploy-record-routes-check-job-local.sh already run
#     once (job body + routes-universe.v1/routes-rounds.v1 artifacts
...
#
# Usage:  ./scripts/enqueue-record-routes-check.sh
set -euo pipefail
...
exec uv run --extra jobs python3 "${REPO_ROOT}/scripts/enqueue_record_routes_check.py" "$@"

#!/usr/bin/env python3
"""Enqueue a contributor-index validation check across every targeted Pi
worker (fanned out one job per worker, aggregated pass/fail), via the jobs
broker + RQ. Re-checks the already-published contributor-index.v1.json/
albums.v1.json pair deployed alongside the job body by
deploy-contributor-index-check-job.yml -- no dataset, no CreditGraph, safe
to run at any time.
...
(deploy-contributor-index-check-job.yml).

Not meant to be invoked directly -- use
scripts/enqueue-contributor-index-check.sh (or
`make contributor-index-check-distributed`), which sources
...
QUEUE_PREFIX = "contributor-index-check"
JOB_TIMEOUT_S = 60
JOB_FUNCTION = "public_artifact_check_job.check_contributor_index"
# Filenames only: deploy-contributor-index-check-job.yml copies both
# artifacts into the same persistent rq_jobs_dir as the job body itself, and
# public_artifact_check_job.py resolves relative paths against that
# directory (see its _resolve()).
CONTRIBUTOR_INDEX_FILENAME = "contributor-index.v1.json"
CATALOG_FILENAME = "albums.v1.json"
USAGE_HINT = "scripts/enqueue-contributor-index-check.sh"
...
def main() -> None:
    ...
    print(
        f"==> Checking {CONTRIBUTOR_INDEX_FILENAME}/{CATALOG_FILENAME} across "
    ...
    per_worker = enqueue_and_collect(
        ...
        job_args=(CONTRIBUTOR_INDEX_FILENAME, CATALOG_FILENAME),
    ...
    output_path = write_report(
        prefix=QUEUE_PREFIX,
        extra={
            "contributor_index_artifact": CONTRIBUTOR_INDEX_FILENAME,
    ...
    if all(v["ok"] for v in per_worker.values()):
        print(f"==> PASS: contributor index is valid on all {len(per_worker)} worker(s).")
    else:

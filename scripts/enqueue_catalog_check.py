...
albums.v1.json artifact deployed alongside the job body by
deploy-catalog-check-job.yml -- no dataset, no CreditGraph, safe to run at
any time.

Mirrors scripts/enqueue_connection_rounds_check.py's broker-connect,
per-worker-queue, burst-worker-launch, and wait-and-collect structure, but
single-artifact (one positional arg to the job function, not a pair) --
see that file's own comments for the "why per-worker queue" rationale.
...
def require_env(name: str) -> str:
    ...
    if not value:
        print(f"ABORT: set {name} (see scripts/enqueue-catalog-check.sh).", file=sys.stderr)
        raise SystemExit(1)
    return value
...
def main() -> None:
    ...
    print(f"==> Checking {CATALOG_FILENAME} via {worker}.")
    job = queue.enqueue(JOB_FUNCTION, CATALOG_FILENAME, job_timeout=JOB_TIMEOUT_S)
    ...
    output_path = OUTPUT_DIR / f"catalog-check-{timestamp}.json"
    output_path.write_text(json.dumps(record, indent=2, default=str) + "\n")

    print(f"==> Wrote {output_path}.")
    if record["ok"]:
        print("==> PASS: public album catalog is valid.")
    else:

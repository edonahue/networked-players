#!/usr/bin/env bash
#
# Enqueue a single contributor-index validation check across the joined Pi
...
# actual driver (scripts/enqueue_contributor_index_check.py).
#
...
#   - infra/ansible/run-deploy-contributor-index-check-job-local.sh already
...
#
# Usage:  ./scripts/enqueue-contributor-index-check.sh
set -euo pipefail
...
exec uv run --extra jobs python3 "${REPO_ROOT}/scripts/enqueue_contributor_index_check.py" "$@"

...
# actual driver (scripts/enqueue_rounds_check.py).
#
...
#   - infra/ansible/run-deploy-rounds-check-job-local.sh already run once
#     (job body + universe.v1/rounds.v1 artifacts deployed to each targeted
#     Pi's rq-jobs directory)
#
# Results are written to local/jobs/ only -- see
# docs/decisions/0018-benchmark-results-local-only.md.
#
# Usage:  ./scripts/enqueue-rounds-check.sh
set -euo pipefail
...
exec uv run --extra jobs python3 "${REPO_ROOT}/scripts/enqueue_rounds_check.py" "$@"

...
# actual driver (scripts/enqueue_cohort_check.py).
#
...
#   - infra/ansible/run-deploy-cohort-check-job-local.sh already run once
#     (job body deployed to each targeted Pi's rq-jobs directory)
#
# Results are written to local/jobs/ only -- see
# docs/decisions/0018-benchmark-results-local-only.md.
#
# Usage:  ./scripts/enqueue-cohort-check.sh --kind connectivity --artifact <path>
#         ./scripts/enqueue-cohort-check.sh --kind playable-cohort --artifact <path>
set -euo pipefail
...
exec uv run --extra jobs python3 "${REPO_ROOT}/scripts/enqueue_cohort_check.py" "$@"

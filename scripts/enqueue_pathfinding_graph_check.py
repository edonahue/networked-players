#!/usr/bin/env python3
"""Enqueue a pathfinding-graph validation check across every targeted Pi
worker (fanned out one job per worker, aggregated pass/fail), via the jobs
broker + RQ. Re-checks the already-published graph.v1.json/albums.v1.json
pair deployed alongside the job body by
deploy-pathfinding-graph-check-job.yml -- no dataset, no CreditGraph, safe
to run at any time.
...
(deploy-pathfinding-graph-check-job.yml).

Not meant to be invoked directly -- use
scripts/enqueue-pathfinding-graph-check.sh (or
`make pathfinding-graph-check-distributed`), which sources
...
QUEUE_PREFIX = "pathfinding-graph-check"
JOB_TIMEOUT_S = 60
JOB_FUNCTION = "public_artifact_check_job.check_pathfinding_graph"
# Filenames only: deploy-pathfinding-graph-check-job.yml copies both
...
PATHFINDING_GRAPH_FILENAME = "pathfinding-graph.v1.json"
CATALOG_FILENAME = "albums.v1.json"
USAGE_HINT = "scripts/enqueue-pathfinding-graph-check.sh"
...
def main() -> None:
    ...
    print(
        f"==> Checking {PATHFINDING_GRAPH_FILENAME}/{CATALOG_FILENAME} across "
    ...
    per_worker = enqueue_and_collect(
        ...
        job_args=(PATHFINDING_GRAPH_FILENAME, CATALOG_FILENAME),
    ...
    output_path = write_report(
        prefix=QUEUE_PREFIX,
        extra={
            "pathfinding_graph_artifact": PATHFINDING_GRAPH_FILENAME,
    ...
    if all(v["ok"] for v in per_worker.values()):
        print(f"==> PASS: pathfinding graph is valid on all {len(per_worker)} worker(s).")
    else:

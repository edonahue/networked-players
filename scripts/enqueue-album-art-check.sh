#!/usr/bin/env bash
#
# Enqueue a single album-art-registry validation check across the joined Pi
...
# actual driver (scripts/enqueue_album_art_check.py).
#
...
#   - infra/ansible/run-deploy-album-art-check-job-local.sh already run
#     once (job body + album-art.v1/albums.v1 artifacts deployed to each
#     targeted Pi's rq-jobs directory)
#
# Results are written to local/jobs/ only -- see
# docs/decisions/0018-benchmark-results-local-only.md.
#
# Usage:  ./scripts/enqueue-album-art-check.sh
set -euo pipefail
...
exec uv run --extra jobs python3 "${REPO_ROOT}/scripts/enqueue_album_art_check.py" "$@"

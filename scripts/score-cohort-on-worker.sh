#!/usr/bin/env bash
# Capability-routed whole-cohort scoring. The broker credential stays in the
# ignored local environment and is never printed or copied into an artifact.
set -euo pipefail
...
cd "${REPO_ROOT}"
exec uv run python scripts/submit_cohort_score.py "$@"

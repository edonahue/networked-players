#!/usr/bin/env python3
"""Enqueue a single Connection-daily-manifest validation check across the
joined Pi workers, via the jobs broker + RQ. Re-checks the already-published
daily-manifest.v1.json/connection-rounds.v1.json pair deployed alongside the
job body by deploy-daily-manifest-check-job.yml -- no dataset, no
CreditGraph, safe to run at any time.
...
(deploy-daily-manifest-check-job.yml).

Not meant to be invoked directly -- use scripts/enqueue-daily-manifest-check.sh
(or `make daily-manifest-check-distributed`), which sources
...
QUEUE_PREFIX = "daily-manifest-check"
JOB_TIMEOUT_S = 60
WAIT_TIMEOUT_S = 120.0
JOB_FUNCTION = "daily_manifest_check_job.check_connection_daily_manifest"
# Filenames only: deploy-daily-manifest-check-job.yml copies both artifacts
# into the same persistent rq_jobs_dir as the job body itself, and
# daily_manifest_check_job.py resolves relative paths against that
# directory (see its _resolve()).
MANIFEST_FILENAME = "daily-manifest.v1.json"
...
def require_env(name: str) -> str:
    ...
    if not value:
        print(
            f"ABORT: set {name} (see scripts/enqueue-daily-manifest-check.sh).",
            file=sys.stderr,
        )
        raise SystemExit(1)
    return value
...
def main() -> None:
    ...
    print(f"==> Checking {MANIFEST_FILENAME}/{ROUNDS_FILENAME} via {worker}.")
    job = queue.enqueue(
        JOB_FUNCTION,
        MANIFEST_FILENAME,
        ROUNDS_FILENAME,
    ...
    output_path = OUTPUT_DIR / f"daily-manifest-check-{timestamp}.json"
    output_path.write_text(json.dumps(record, indent=2, default=str) + "\n")

    print(f"==> Wrote {output_path}.")
    if record["ok"]:
        print("==> PASS: Connection-daily-manifest is valid.")
    else:

#!/usr/bin/env python3
"""Enqueue a Connection-daily-manifest validation check across every
...
USAGE_HINT = "scripts/enqueue-daily-manifest-check.sh"
...
def main() -> None:
    ...
    print(
        f"==> Checking {MANIFEST_FILENAME}/{ROUNDS_FILENAME} across "
    ...
    per_worker = enqueue_and_collect(
        ...
        job_args=(MANIFEST_FILENAME, ROUNDS_FILENAME),
        job_timeout=JOB_TIMEOUT_S,
        queue_factory=lambda name: Queue(name, connection=redis_conn),
    )
    output_path = write_report(
        prefix=QUEUE_PREFIX,
        extra={"manifest_artifact": MANIFEST_FILENAME, "rounds_artifact": ROUNDS_FILENAME},
        per_worker=per_worker,
    )

    print(f"==> Wrote {output_path}.")
    if all(v["ok"] for v in per_worker.values()):
        print(f"==> PASS: Connection-daily-manifest is valid on all {len(per_worker)} worker(s).")
    else:

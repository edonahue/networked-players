...
# the joined Pi workers. Thin wrapper: sources local/jobs-broker.env
# (written by infra/swarm/deploy-jobs-broker.sh) to build JOBS_BROKER_URL,
# then runs the actual driver (scripts/enqueue_connection_rounds_check.py).
#
...
#   - infra/ansible/run-deploy-connection-rounds-check-job-local.sh already
#     run once (job body + connection-universe.v1/connection-rounds.v1
#     artifacts deployed to each targeted Pi's rq-jobs directory)
#
# Results are written to local/jobs/ only -- see
# docs/decisions/0018-benchmark-results-local-only.md.
#
# Usage:  ./scripts/enqueue-connection-rounds-check.sh
set -euo pipefail
...
exec uv run --extra jobs python3 "${REPO_ROOT}/scripts/enqueue_connection_rounds_check.py" "$@"

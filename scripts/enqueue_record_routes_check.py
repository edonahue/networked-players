#!/usr/bin/env python3
"""Enqueue a single Record Routes artifact validation check across the
...
def require_env(name: str) -> str:
    ...
    if not value:
        print(
            f"ABORT: set {name} (see scripts/enqueue-record-routes-check.sh).",
            file=sys.stderr,
        )
        raise SystemExit(1)
    return value
...
def main() -> None:
    ...
    output_path = OUTPUT_DIR / f"record-routes-check-{timestamp}.json"
    output_path.write_text(json.dumps(record, indent=2, default=str) + "\n")

    print(f"==> Wrote {output_path}.")
    if record["ok"]:
        print("==> PASS: Record Routes pool is valid.")
    else:
